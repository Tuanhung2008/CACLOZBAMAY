...
class User extends Entity implements LinkableInterface
{
	...
	public function canFollowUser(User $user)
	{
		...
		if (!$this->isInActiveUserState($user))
		{
			return false;
		}
	...
	public function isInActiveUserState(User $user = null): bool
	{
		if (!$user)
		{
			$user = $this;
	...
	public function canChangeUsername(&$error = null): bool
	{
		...
		if ($this->hasPermission('general', 'changeUsername'))
		{
			if ($this->user_state != 'valid')
			{
				$error = \XF::phrase('only_valid_users_may_change_their_username');
				return false;
			}
	...
	)
	{
		...
		$result->user_title = $this->custom_title !== ''
			? $this->custom_title
			: $this->app()->templater()->getDefaultUserTitleForUser($this);
	...
	public function getLdStructuredData(array $extraData = []): array
	{
		$options = $this->app()->options();
		$userLink = $this->getContentUrl(true);

		$mainEntity = [
			'@type' => 'Person',
			'@id' => $userLink,
			'name' => $this->username,
			'description' => $this->custom_title
				?: $this->app()->templater()->getDefaultUserTitleForUser($this),
			'identifier' => $this->user_id,
			'url' => $userLink,
			'image' => $this->getAvatarUrl('o', null, true),
			'agentInteractionStatistic' => [
				[
					'@type' => 'InteractionCounter',
					'interactionType' => 'https://schema.org/FollowAction',
					'userInteractionCount' => count($this->Profile->following),
				],
				[
					'@type' => 'InteractionCounter',
					'interactionType' => 'https://schema.org/WriteAction',
					'userInteractionCount' => $this->message_count,
				],
			],
			'interactionStatistic' => [
				[
					...
					'userInteractionCount' => $this->reaction_score,
				],
			],
		];

		$likedCount = $extraData['liked_count'] ?? null;
		if ($likedCount)
		{
			$mainEntity['agentInteractionStatistic'][] = [
				'@type' => 'InteractionCounter',
				'interactionType' => 'https://schema.org/LikeAction',
				'userInteractionCount' => $likedCount,
			];
		}

		$followersCount = $extraData['followers_count'] ?? null;
		if ($followersCount)
		{
		...
		{
			$sameAs = [];
			$fields = $this->app()->getCustomFields(
				'users',
				'contact',
				null,
				['profile']
			);
			$set = $this->Profile->custom_fields;
			foreach ($fields AS $fieldId => $fieldDefinition)
			{
				if ($fieldDefinition['match_type'] !== 'url')
				{
					continue;
				}

				if (!$fieldDefinition->hasValue($set[$fieldId]))
				{
					continue;
				}
		...
		$structuredData = [
			...
			'@type' => 'ProfilePage',
			'url' => $userLink,
			'dateCreated' => gmdate('c', $this->register_date),
