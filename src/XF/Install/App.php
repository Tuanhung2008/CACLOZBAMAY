...
class App extends \XF\App
{
	...
	protected function getInstallerPhrases()
	{
		return [
			...
			'php_version_x_outdated_upgrade' => 'Your server is running an outdated and unsupported version of PHP ({version}). If possible, you should upgrade to PHP 5.6 or newer (we recommend PHP 8.1) to benefit from improved security and performance.',
			...
			'php_max_int_size_warning' => 'You PHP install does not support 64-bit integers. Please ask your host to upgrade to a 64-bit version of PHP.',
	...
	{
		$templateName = $params['templateName'] ?? 'PAGE_CONTAINER';
		if (!$templateName)
		{
			return $content;
		}
