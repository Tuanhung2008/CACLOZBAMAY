...
class Html extends AbstractRenderer
{
	...
	public function renderTagAttach(array $children, $option, array $tag, array $options)
	{
		...
		return trim($this->getRenderedAttachment($attachment, $viewParams));
	}
	...
	public function renderTagImage(array $children, $option, array $tag, array $options)
	{
		...
		return trim($this->getRenderedImg($imageUrl, $validUrl, $params));
	}
	...
	public function renderTagMedia(array $children, $option, array $tag, array $options)
	{
		...
		if ($this->isLegacyMediaSite($mediaSiteId))
		{
			return $this->renderLegacyMediaSite($mediaSiteId, $mediaKey);
		}
		...
		$cookieThirdParties = array_filter(preg_split(
			'/\r?\n/',
		...
		if ($cookieThirdParties)
		{
			...
			if ($unconsentedThirdParties)
			{
				return $this->templater->renderMacro(
					'public:misc_cookies',
					'third_party_consent_form',
					[
						'unconsented' => $unconsentedThirdParties,
						'embedHtml' => $embedHtml,
						'siteId'    => $mediaSiteId,
						'id'        => $keyEncoded,
					]
	...
	protected function isLegacyMediaSite(string $mediaSiteId): bool
	{
		return in_array($mediaSiteId, $this->legacyMediaSites);
	}

	protected function renderLegacyMediaSite(string $mediaSiteId, string $mediaKey): string
	{
		return $this->templater->renderTemplate('public:_media_site_legacy_embed', [
			'siteId'   => $mediaSiteId,
			'id'       => $mediaKey,
		]);
	}
