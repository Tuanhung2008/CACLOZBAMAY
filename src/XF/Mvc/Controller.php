...
abstract class Controller
{
	 ...
	 */
	public function assertCookieConsent(array $groups, array $thirdParties)
	{
		$cookieConsent = $this->app->cookieConsent();
		$unconsentedGroups = $cookieConsent->getUnconsentedGroups($groups);
		$unconsentedThirdParties = $cookieConsent->getUnconsentedThirdParties(
			$thirdParties
		);
		if (!$unconsentedGroups && !$unconsentedThirdParties)
		{
			return;
		}
		...
		$viewParams = [
			'unconsentedGroups' => $unconsentedGroups,
			'unconsentedThirdParties' => $unconsentedThirdParties,
			'redirect' => $redirect
		];
		$view = $this->view(
			'XF:Misc\Cookies',
	...
	public function assertCaptchaCookieConsent(
		string $class = null,
		bool $force = false
	)
	{
		...
		$this->assertCookieConsent(
			[],
			$cookieConsent->getCaptchaThirdParties($class, $force)
		);
	}
	...
	protected function getCurrentUrlWithoutAjaxParams(): string
	{
		$routePath = $this->request->getRoutePath();
		$queryParams = $this->request->getRequestQueryParamsExcept(
			[
				'_xfRequestUri',
				'_xfWithData',
				'_xfToken',
		...
		return $this->router()->buildFinalUrl(
			null,
			$routePath,
			$queryParams
		);
	...
	public function assertDraftsEnabled()
	{
		$this->assertPostOnly();
	...
	/**
	 * @param string|\Stringable|array<string|\Stringable> $error
	...
	{
		return $this->assertRecordExistsByUnique($identifier, $id, null, $with, $phraseKey);
	}
	...
	{
		return call_user_func_array([$this->app, 'helper'], func_get_args());
	}
	...
	{
		return $this->app->router()->buildPaginatedLink($link, $data, $page, $parameters);
	}
	...
	public function captchaResponse($class = null)
	{
		$captcha = $this->app->captcha($class);
