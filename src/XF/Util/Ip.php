...
class Ip
{
	...
	public static function convertIpStringToBinary($ip)
	{
		...
		if (strpos($ip, ':') !== false)
		{
			...
			if (preg_match('/^00000000000000000000ffff([0-9a-f]{8})$/', $hex, $match))
			{
				...
				return static::convertHexToBin($match[1]);
			}

			return static::convertHexToBin($hex);
		}
	...
	{
		$range = static::getIpCidrMatchRange($rangeIp, $cidr);
		if (is_string($range))
		{
			return ($testIp === $range);
		}
	 ...
	 * @return bool
	 */
	public static function ipMatchesRange(
		string $testIp,
		string $lowerBound,
		string $upperBound
	): bool
	...
	public static function checkIpsAgainstBinaryRangeList($checkIps, array $firstByteRangeList)
	{
		...
		foreach ($checkIps AS $ip)
		{
			...
			{
				if (static::ipMatchesRange($binary, $range[0], $range[1]))
				{
					return $range;
				}
			}
	...
	public static function parseIpRangeString($ip)
	{
		...
		if (strpos($ip, ':') !== false)
		{
			...
			$binary = static::convertIpStringToBinary($ip);
			if ($binary === false)
			{
				return false;
			}
		...
		else
		{
			...
			$binary = static::convertIpStringToBinary(implode('.', $ipParts));
			if (!$binary)
			{
				return false;
			}
		...
		$range = static::getIpCidrMatchRange($binary, $cidr);
	...
	{
		if (isset(static::$lookupCache[$ip]))
		{
			return static::$lookupCache[$ip];
		}

		$decompressed = static::convertIpBinaryToString($ip, false);
		...
		if (strpos($ip, ':') !== false)
		{
			...
			$checkIp = static::convertIpBinaryToString($binary, false);
			if (!$checkIp)
			{
				return false;
			}
